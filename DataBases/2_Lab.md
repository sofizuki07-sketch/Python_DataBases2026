...
### Задание 4
Получилось добавить данные во все отношения, кроме orders и deliveries (последнюю пришлось даже удалить и заново попробовать заполнить...). Они самые "зависимые" от других отношений.
...
Скорее всего, дело в связях 1:1 таблиц orders и deliveries
Я так и не разобралась, как грамотно сделать связь, чтобы в deliveries отражались пары id заказа и фамилия(номер) заказчика в соответствии с заказом.
В статье на Хабр по данной БД не описано конктерно, как эти связи оформить(между чем и чем, как)
